...
// Backend principal (server.js) - VERSÃO ESTÁVEL E CORRIGIDA
...
const RADIUS_CLIENT_MODE = (process.env.RADIUS_CLIENT_MODE || 'global').toLowerCase();
const RADIUS_GLOBAL_SECRET = process.env.RADIUS_GLOBAL_SECRET || '';
const RADIUS_VPN_SERVER_IP = process.env.RADIUS_VPN_SERVER_IP || '10.254.1.1';
const RADIUS_GLOBAL_FALLBACK_SECRET = process.env.RADIUS_GLOBAL_FALLBACK_SECRET || RADIUS_GLOBAL_SECRET || '';

// Constantes VPN (legado)
...
async function revokeAccess(macAddress, popIp = '192.168.32.1', apiUser = null, apiPass = null, popId = null) {
  ...
  try {
    ...
    if (!username || !password) throw new Error('Credenciais da API MikroTik não disponíveis');
...
async function authorizeAccess(macAddress, popIp = '192.168.32.1', apiUser = null, apiPass = null, popId = null, durationMinutes = 15, speedMbps = null, planName = 'free_trial', durationSeconds = null) {
  ...
  if (!macAddress) return { success: false, viaApi: false, viaRadius: false, errors: ['MAC inválido'] };

  let viaApi = false;
  let viaRadius = false;
  const errors = [];

  // Tentativa via API MikroTik (IP Binding)
  if (RouterOSAPI) {
    try {
      let username = apiUser;
      let password = apiPass;
  ...
  // Tentativa via RADIUS (radius_replies)
  try {
    ...
    const expiresAt = new Date(Date.now() + sessionTimeoutSeconds * 1000).toISOString();
    ...
    const upsertRadiusReply = async (payload) => {
      ...
      // Fallback para schemas sem unique constraint
      if (attempt.error.message.includes('no unique or exclusion constraint')) {
        await supabase.from('radius_replies').delete().eq('username', payload.username).eq('attribute', payload.attribute);
...
async function findOrCreateHotspotUser({ macAddress, ipAddress = null, planName = 'free_trial', status = 'trial', popId = null, expiresAt = null }) {
  ...
  const base = { username: cleanMac, mac_address: cleanMac, updated_at: now };
  const optional = {
  ...
  if (existing) {
    ...
    const hasActivePlan = (existing.status === 'active' || existing.status === 'paid' || existing.status === 'vip') &&
      existing.plan_name && existing.plan_name !== 'free_trial' &&
    ...
    for (let i = 0; i < 4; i++) {
      const { data, error } = await supabase.from('users').update(updatePayload).eq('id', existing.id).select('*').maybeSingle();
...
function normalizeFreeTrialConfig(value = {}) {
  const durationRaw = firstFiniteNumber(value.duration_seconds, value.duration_minutes !== undefined ? Number(value.duration_minutes) * 60 : undefined, 900);
  const cooldownRaw = firstFiniteNumber(value.cooldown_seconds, value.cooldown_minutes !== undefined ? Number(value.cooldown_minutes) * 60 : undefined, value.cooldown_hours !== undefined ? Number(value.cooldown_hours) * 3600 : undefined, value.reuse_cooldown_hours !== undefined ? Number(value.reuse_cooldown_hours) * 3600 : undefined, 86400);
  return {
...
async function getFreeTrialConfig() {
  try {
    ...
    await registerSystemLog('error', 'free_trial', 'Erro ao carregar configuração de teste grátis', { error: error.message });
...
async function handleFreeTrialAccess({ macAddress, ipAddress = null, popId = null, popIp = null }) {
  ...
  try {
    ...
    await registerSystemLog('error', 'free_trial', 'Erro ao verificar plano ativo', { mac: cleanMac, error: error.message });
  }

  // Verifica sessão ativa
  try {
    const { data: session } = await supabase.from('hotspot_sessions').select('*').in('mac_address', getMacVariants(cleanMac)).eq('status', 'active').gt('expires_at', nowIso).order('created_at', { ascending: false }).limit(1).maybeSingle();
    if (session) {
    ...
    await registerSystemLog('error', 'free_trial', 'Erro ao verificar sessão ativa', { mac: cleanMac, error: error.message });
  }

  // Cooldown
  let previousTrial = null;
  try {
    const { data: ft } = await supabase.from('free_trials').select('*').in('mac_address', getMacVariants(cleanMac)).order('updated_at', { ascending: false }).limit(1).maybeSingle();
    ...
    await registerSystemLog('error', 'free_trial', 'Erro ao verificar cooldown', { mac: cleanMac, error: error.message });
  }
  ...
  try {
    ...
    await registerSystemLog('error', 'free_trial', 'RADIUS liberado, mas falhou ao gravar sessão', { mac: cleanMac, error: error.message });
  }
  ...
  try {
    ...
    await registerSystemLog('error', 'free_trial', 'RADIUS liberado, mas falhou ao gravar histórico', { mac: cleanMac, error: error.message });
  }
...
// ============================================================
// 🧹 CRON DE LIMPEZA (a cada 1 minuto)
// ============================================================

setInterval(async () => {
  try {
    const now = new Date().toISOString();
    const { data: expiredSessions } = await supabase.from('hotspot_sessions').select('id, mac_address, pop_ip').eq('status', 'active').lt('expires_at', now);
    ...
    const { data: expiredRadius } = await supabase.from('radius_replies').select('username').eq('status', 'active').lt('expires_at', now);
...
app.post('/api/auth/login', loginLimiter, async (req, res) => {
  try {
    ...
    if (admin.password !== hashedPassword && admin.password !== password) return res.status(401).json({ error: 'Credenciais inválidas' });
...
app.put('/api/profile', authMiddleware, async (req, res) => {
  try {
    ...
    if (new_password) {
      ...
      if (admin.password !== hashedCurrent && admin.password !== current_password) return res.status(401).json({ error: 'Senha atual incorreta' });
    }
    const updateData = { updated_at: new Date().toISOString() };
    if (username) updateData.username = username;
    if (email) updateData.email = email;
    if (new_password) updateData.password = crypto.createHash('sha256').update(new_password).digest('hex');
    const { data, error: updateError } = await supabase.from('admins').update(updateData).eq('id', adminId).select().single();
...
app.get('/api/perfil', authMiddleware, (req, res, next) => { req.url = '/api/profile'; next(); });
...
app.get('/api/users', authMiddleware, async (req, res) => {
  try {
    ...
    if (search) query = query.or(`name.ilike.%${search}%,username.ilike.%${search}%,mac_address.ilike.%${search}%,phone.ilike.%${search}%,cpf.ilike.%${search}%`);
...
app.post('/api/users', authMiddleware, async (req, res) => {
  try {
    ...
    const payload = {
      name, username: username || cleanMac || undefined, mac_address: cleanMac || null, phone, cpf, email, address,
      plan_id: plan ? plan.id : (plan_id || null), plan_name: plan ? plan.name : (plan_name || null), hotspot_id,
      status: plan ? 'active' : (status || 'inactive'), is_vip: is_vip || false,
      ...(expiresAt ? { expires_at: expiresAt } : {}), created_at: now.toISOString(), updated_at: now.toISOString()
    };
    ...
    await registerAuditLog(req.user.username, 'create', 'user', `Usuário criado: ${name || cleanMac || data.id}`, getClientIp(req), req.headers['user-agent']);
...
app.put('/api/users/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (data.mac_address && data.status === 'active' && data.plan_name && data.plan_name !== 'free_trial') {
      ...
      const durationSeconds = data.expires_at ? Math.max(10, Math.ceil((new Date(data.expires_at).getTime() - Date.now()) / 1000)) : Number(activePlan?.duration_days || 30) * 24 * 60 * 60;
...
app.post('/api/users/:id/renew', authMiddleware, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('users').update({ plan_id, plan_name: plan.name, status: 'active', expires_at: expiresAt, updated_at: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
    if (data.mac_address) {
      const durationSeconds = Math.max(10, Math.ceil((new Date(expiresAt).getTime() - Date.now()) / 1000));
...
app.get('/api/users/export', authMiddleware, async (req, res) => {
  try {
    ...
    const rows = (data || []).map(u => [u.name || '', u.username || '', u.mac_address || '', u.phone || '', u.cpf || '', u.email || '', u.plan_name || '', u.status || '', u.created_at ? new Date(u.created_at).toLocaleDateString('pt-BR') : '']);
...
app.post('/api/plans', authMiddleware, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('plans').insert({ name, price, speed_mbps, duration_days, description, active: active !== undefined ? active : true }).select().single();
    if (error) throw error;
...
// ============================================================
// 💳 ROTAS DE PAGAMENTOS (MERCADO PAGO - SEM MOCK)
// ============================================================
...
app.post('/api/payments/generate-pix', paymentLimiter, async (req, res) => {
  try {
    ...
    if (plan.active === false) return res.status(400).json({ error: 'Plano indisponível' });
    ...
    let result;
    if (payment_id) {
      result = await supabase.from('payments').update(paymentData).eq('id', payment_id).select().single();
    } else {
...
app.get('/api/check-payment', async (req, res) => {
  try {
    ...
    if (macFilter) {
      const { data: payment, error } = await supabase.from('payments').select('*').in('user_mac', getMacVariants(macFilter)).order('created_at', { ascending: false }).limit(1).maybeSingle();
      if (error) throw error;
...
app.post('/api/vouchers', authMiddleware, async (req, res) => {
  try {
    ...
    if (qty === 1) {
      const { data, error } = await supabase.from('vouchers').insert({ code: crypto.randomBytes(4).toString('hex').toUpperCase(), plan_name, amount: parseFloat(amount) || 0, expires_at: expires_at ? new Date(expires_at).toISOString() : null, duration_hours: duration_hours || 24, status: 'active', used: false, created_at: new Date().toISOString() }).select().single();
      if (error) throw error;
      res.status(201).json(data);
    } else {
      const vouchers = [];
      for (let i = 0; i < Math.min(qty, 100); i++) {
        vouchers.push({ code: crypto.randomBytes(4).toString('hex').toUpperCase(), plan_name, amount: parseFloat(amount) || 0, expires_at: expires_at ? new Date(expires_at).toISOString() : null, duration_hours: duration_hours || 24, status: 'active', used: false, created_at: new Date().toISOString() });
      }
...
app.post('/api/vouchers/validate', voucherLimiter, async (req, res) => {
  try {
    ...
    await supabase.from('vouchers').update({ status: 'used', used: true, used_at: new Date().toISOString(), mac_address: cleanMac || null, updated_at: new Date().toISOString() }).eq('id', voucher.id);
...
async function safeInsertWithFallback(table, preferredPayload, fallbackPayload) {
  ...
  if (result.error && fallbackPayload) result = await supabase.from(table).insert(fallbackPayload).select().single();
  return result;
}
...
function buildL2tpChapSecrets(pops) {
  const header = ['# ======================================================================', '# AUTO-GENERATED BLOCK - MS TELECOM POPs (DO NOT EDIT INSIDE)', `# Generated at: ${new Date().toISOString()}`, '# ======================================================================', ''].join('\n');
  const lines = (pops || []).filter(p => p && p.vpn_enabled === true && String(p.vpn_type || '').toLowerCase() === 'l2tp_ipsec').filter(p => p.vpn_username && p.vpn_password && (p.vpn_ip || p.radius_client_ip)).map(p => {
...
async function syncL2tpChapSecretsFromDb() {
  try {
    const { data: pops, error } = await supabase.from('pops').select('id, unique_id, vpn_enabled, vpn_type, vpn_ip, radius_client_ip, vpn_username, vpn_password').order('unique_id', { ascending: true });
    if (error) throw error;
...
async function allocateNextVpnIp() {
  ...
  if (start === null || end === null || start >= end) throw new Error('Invalid VPN IP pool range');
...
async function ensurePopProvisioningMaterial(pop) {
  ...
  if (!nextVpnIp) nextVpnIp = await allocateNextVpnIp();
  if (!pop.unique_id || !pop.radius_secret || !pop.vpn_ip || !pop.vpn_username || !pop.vpn_password || pop.vpn_enabled === undefined || pop.vpn_enabled === null || !pop.vpn_type) {
    const { data: updated, error } = await supabase.from('pops').update({ unique_id: uniqueId, radius_secret: nextRadiusSecret, vpn_enabled: nextVpnEnabled, vpn_type: nextVpnType, vpn_ip: nextVpnIp, vpn_username: nextVpnUsername, vpn_password: nextVpnPassword, updated_at: now }).eq('id', popId).select('*').single();
    if (error) throw error;
    pop = updated;
  } else {
  ...
  const { data: existingCreds } = await supabase.from('mikrotik_credentials').select('*').eq('pop_id', popId).maybeSingle();
  ...
  if (!existingCreds?.api_user || !existingCreds?.api_pass || existingCreds?.pop_ip !== (pop.ip || null)) {
    await upsertMikrotikCredentials({ pop_id: popId, pop_ip: pop.ip || null, api_user: apiUser, api_pass: apiPass });
  }
  return { ...pop, api_user: apiUser, api_pass: apiPass, radius_secret: pop.radius_secret || nextRadiusSecret, vpn_enabled: pop.vpn_enabled ?? nextVpnEnabled, vpn_type: pop.vpn_type || nextVpnType, vpn_ip: pop.vpn_ip || nextVpnIp, vpn_username: pop.vpn_username || nextVpnUsername, vpn_password: pop.vpn_password || nextVpnPassword };
}

// ============================================================
// 🏢 ROTAS DE POPS (HOTSPOTS)
// ============================================================
...
app.get('/api/pops', authMiddleware, async (req, res) => {
  try {
    ...
    const normalized = (data || []).map(p => {
      ...
      const isOnline = diffSec >= 0 && diffSec <= 90;
...
app.post('/api/pops', authMiddleware, async (req, res) => {
  try {
    ...
    if (normalized.vpn_password === '' || normalized.vpn_password === null || typeof normalized.vpn_password === 'undefined' || String(normalized.vpn_password).trim() === '********') delete normalized.vpn_password;
    ...
    delete preferred.id;
    const fallback = { name: normalized.name, ip: normalized.ip || null, location: normalized.location || null, status: 'offline', created_at: now, updated_at: now };
    ...
    try {
      await supabase.from('settings').upsert({ key: `pop_config_${data.id}`, value: normalized, updated_at: now }, { onConflict: 'key' });
    } catch (_e) {}
...
function buildPopInstallScript(pop, config = {}) {
  ...
  if (!apiUser || !apiPass || !radiusSecret) throw new Error('Missing persisted POP credentials');
  const wanInterface = config.wan_interface || 'ether1';
  const lanInterface = config.lan_interface || 'ether2';
  const wanType = config.wan_type || 'dhcp';
  ...
  let installationType = String(config.installation_type || config.installation || 'new').toLowerCase();
  if (installationType === 'existing') installationType = 'production';
  const isTrunk = installationType === 'trunk';
  const trunkTopology = String(config.trunk_topology || 'single').toLowerCase();
  ...
  let vpnBlock = '';
  if (vpnEnabled) {
    if (RADIUS_CLIENT_MODE !== 'vpn_legacy') throw new Error('vpn_enabled=true requires RADIUS_CLIENT_MODE=vpn_legacy');
    if (!vpnIp || !vpnUsername || !vpnPassword) throw new Error('Missing POP VPN fields');
    if (vpnType === 'l2tp_ipsec') {
      vpnBlock = `# VPN (L2TP/IPsec - RouterOS v6)\n/ppp profile add name="MS-VPN" use-encryption=yes comment="${tag}"\n/interface l2tp-client add name="ms-vpn-${popId}" connect-to=${VPN_PUBLIC_ENDPOINT} user="${vpnUsername}" password="${vpnPassword}" profile="MS-VPN" use-ipsec=yes ipsec-secret="${VPN_L2TP_IPSEC_PSK}" add-default-route=no keepalive-timeout=30 disabled=no comment="${tag}"\n:delay 2s\n/ip service set api address=${VPN_INTERNAL_RADIUS_IP}/32\n:delay 500ms\n`;
    } else if (vpnType === 'sstp') {
      vpnBlock = `# VPN (SSTP - RouterOS v6)\n/ppp profile add name="MS-VPN" use-encryption=yes comment="${tag}"\n/interface sstp-client add name="ms-vpn-${popId}" connect-to=${VPN_PUBLIC_ENDPOINT} user="${vpnUsername}" password="${vpnPassword}" profile="MS-VPN" add-default-route=no disabled=no comment="${tag}"\n:delay 2s\n/ip service set api address=${VPN_INTERNAL_RADIUS_IP}/32\n:delay 500ms\n`;
    } else throw new Error('Unsupported vpn_type');
  }
  const radiusClientIp = String(pop.vpn_ip || pop.radius_client_ip || '').trim();
  const radiusVpnBlock = radiusClientIp ? `/radius add service=hotspot address=${RADIUS_VPN_SERVER_IP} src-address=${radiusClientIp} secret="${radiusSecret}" authentication-port=1812 accounting-port=1813 timeout=3s domain="${popId}" protocol=udp comment="${tag}-radius-vpn"\n` : `# VPN IP ausente: RADIUS via VPN não gerado\n`;
  const radiusPublicFallbackBlock = RADIUS_GLOBAL_FALLBACK_SECRET ? `/radius add service=hotspot address=${RADIUS_SERVER_IP} secret="${RADIUS_GLOBAL_FALLBACK_SECRET}" authentication-port=1812 accounting-port=1813 timeout=5s domain="${popId}" protocol=udp comment="${tag}-radius-public-fallback"\n` : '';
  const radiusBlock = radiusVpnBlock + radiusPublicFallbackBlock + `/radius incoming set accept=yes\n:delay 1s\n`;
  const wgHosts = ['hotspot-system.vercel.app', '*.vercel.app', 'vercel.app', 'cdn.vercel.app', 'mstelecom-api.duckdns.org', 'api.mercadopago.com', 'mercadopago.com', 'www.mercadopago.com', 'cdnjs.cloudflare.com', 'unpkg.com', 'fonts.googleapis.com', 'fonts.gstatic.com'];
  ...
  const vlanLine = trunkClientSingleEnabled ? `/interface vlan add name="ms-vlan-${trunkClientVlanId}" interface=${trunkUplinkInterface} vlan-id=${trunkClientVlanId} comment="${tag}"\n` : trunkClientDualEnabled ? `/interface vlan add name="ms-vlan-${vlanId}" interface=${lanInterface} vlan-id=${vlanId} comment="${tag}"\n` : (!isTrunk && vlanId) ? `/interface vlan add name="ms-vlan-${vlanId}" interface=${lanInterface} vlan-id=${vlanId} comment="${tag}"\n` : '';
  const clientIface = trunkClientSingleEnabled ? `"ms-vlan-${trunkClientVlanId}"` : (trunkClientDualEnabled || (!isTrunk && vlanId)) ? `"ms-vlan-${vlanId}"` : lanInterface;
  ...
  const setParts = `${sessionTime ? ` session-timeout=${sessionTime}` : ''}${rateLimit ? ` rate-limit=${rateLimit}` : ''}${sharedUsers > 0 ? ` shared-users=${sharedUsers}` : ''}`;
  const userProfileTuningLine = `# Perfil de usuario (POP especifico - nao altera o default global)\n:if ([:len [/ip hotspot user profile find name="${profileName}"]] = 0) do={\n  /ip hotspot user profile add name="${profileName}"${setParts} comment="${tag}"\n} else={\n  /ip hotspot user profile set [find name="${profileName}"]${setParts}\n}\n`;
...
app.put('/api/pops/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const updateData = { updated_at: new Date().toISOString() };
    const body = { ...(req.body || {}) };
    if (body.vpn_password === '' || body.vpn_password === null || typeof body.vpn_password === 'undefined' || String(body.vpn_password).trim() === '********') delete body.vpn_password;
...
app.delete('/api/pops/:id', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    await supabase.from('mikrotik_credentials').delete().eq('pop_id', id).catch(() => {});
    await supabase.from('settings').delete().eq('key', `pop_config_${id}`).catch(() => {});
...
app.post('/api/admin/pops/vpn-backfill', authMiddleware, async (req, res) => {
  try {
    ...
    for (const pop of pops || []) {
      const needs = !pop.vpn_ip || !pop.vpn_username || !pop.vpn_password || !pop.vpn_type || pop.vpn_enabled === undefined || pop.vpn_enabled === null;
      if (!needs) continue;
    ...
    const l2tpSync = await syncL2tpChapSecretsFromDb();
    const radiusSync = await syncFreeradiusClientsFromDb();
    res.json({ success: true, updated_count: updated.length, updated, l2tp_sync: l2tpSync.ok ? 'ok' : 'failed', freeradius_sync: radiusSync.ok ? 'ok' : 'failed' });
  } catch (err) {
...
app.post('/api/pops/:id/ping', async (req, res) => {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(pop, 'users_connected') && req.body && typeof req.body.users_connected !== 'undefined') updateData.users_connected = req.body.users_connected;
    if (Object.prototype.hasOwnProperty.call(pop, 'bandwidth') && req.body && typeof req.body.bandwidth !== 'undefined') updateData.bandwidth = req.body.bandwidth;
...
app.post('/api/pops/register', async (req, res) => {
  try {
    ...
    if (error) throw error;
    try {
      await supabase.from('settings').upsert({ key: `pop_token_${data.id}`, value: { token: popToken }, updated_at: now }, { onConflict: 'key' });
    } catch (_e) {}
...
// ============================================================
// 📊 DASHBOARD E ESTATÍSTICAS (resumidas)
// ============================================================
...
app.get('/api/stats/summary', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ total_users: users?.length || 0, active_sessions: activeSessions?.length || 0, total_revenue: totalRevenue, online_pops: 1 });
  } catch (err) {
...
app.get('/api/stats/users-per-hour', authMiddleware, async (req, res) => {
  try {
    ...
    const { data: sessions, error } = await supabase.from('hotspot_sessions').select('created_at').gte('created_at', last24h);
    if (error) throw error;
...
app.get('/api/sessions/active', authMiddleware, async (req, res) => {
  try {
    const now = new Date().toISOString();
    const { data, error } = await supabase.from('hotspot_sessions').select('*').eq('status', 'active').gt('expires_at', now);
    if (error) throw error;
    res.json(data || []);
  } catch (err) {
...
app.get('/api/health', (_req, res) => {
  res.json({ status: 'ok', version: '4.0.0', timestamp: new Date().toISOString(), uptime: process.uptime(), mikrotik_api: !!RouterOSAPI });
});
...
app.get('/api/test-ip', (req, res) => {
  res.json({ ip: getClientIp(req), x_forwarded_for: req.headers['x-forwarded-for'] || null, remote_address: req.socket?.remoteAddress || null });
});
...
app.put('/api/settings/fields', authMiddleware, async (req, res) => {
  try {
    ...
    const { error } = await supabase.from('settings').upsert({ key: 'registration_fields', value: fields, updated_at: new Date().toISOString() }, { onConflict: 'key' });
    if (error) throw error;
...
app.put('/api/settings', authMiddleware, async (req, res) => {
  try {
    const { error } = await supabase.from('settings').upsert({ key: 'system', value: req.body, updated_at: new Date().toISOString() });
    if (error) throw error;
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.put('/api/settings/payment', authMiddleware, async (req, res) => {
  try {
    const { error } = await supabase.from('settings').upsert({ key: 'payment', value: req.body, updated_at: new Date().toISOString() });
    if (error) throw error;
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.put('/api/settings/integrations', authMiddleware, async (req, res) => {
  try {
    const { error } = await supabase.from('settings').upsert({ key: 'integrations', value: req.body, updated_at: new Date().toISOString() });
    if (error) throw error;
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.put('/api/settings/free_trial', authMiddleware, async (req, res) => {
  try {
    const value = normalizeFreeTrialConfig(req.body);
    const { error } = await supabase.from('settings').upsert({ key: 'free_trial', value, updated_at: new Date().toISOString() }, { onConflict: 'key' });
    if (error) throw error;
    res.set('Cache-Control', 'no-store');
...
app.post('/api/webhooks/mercadopago', async (req, res) => {
  try {
    ...
    if (type === 'payment' || action === 'payment.created' || action === 'payment.updated') {
      ...
      const MP_TOKEN = process.env.MERCADOPAGO_ACCESS_TOKEN || process.env.MP_ACCESS_TOKEN;
      const mpResponse = await fetch(`https://api.mercadopago.com/v1/payments/${paymentId}`, { headers: { 'Authorization': `Bearer ${MP_TOKEN}` } });
      if (!mpResponse.ok) throw new Error('Falha ao buscar pagamento no Mercado Pago');
      const mpData = await mpResponse.json();
      if (mpData.status === 'approved') {
        const { data: payment, error: payError } = await supabase.from('payments').select('*').eq('mercado_pago_id', String(paymentId)).maybeSingle();
        if (payError) throw payError;
        if (payment && payment.status !== 'approved') {
          await supabase.from('payments').update({ status: 'approved', approved_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', payment.id);
          ...
          await authorizeAccess(cleanMac, '192.168.32.1', null, null, null, durationMinutes, plan?.speed_mbps || 10, payment.plan_name);
          await supabase.from('hotspot_sessions').insert({ mac_address: cleanMac, plan_name: payment.plan_name, status: 'active', expires_at: new Date(Date.now() + durationMinutes * 60000).toISOString(), created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
          ...
          for (const wh of internalWebhooks || []) {
            fetch(wh.url, { method: wh.method || 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ event: 'payment.confirmed', payment_id: payment.id, mac: payment.user_mac }) }).catch(err => console.error(`❌ Erro ao disparar webhook ${wh.name}:`, err.message));
          }
...
app.post('/api/webhooks', authMiddleware, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('webhooks').insert([{ name, event, url, method, target, active }]).select();
    if (error) throw error;
...
app.post('/api/webhooks/:id', authMiddleware, async (req, res) => {
  try {
    const { name, event, url, method, target, active } = req.body;
    const { data, error } = await supabase.from('webhooks').update({ name, event, url, method, target, active }).eq('id', req.params.id).select();
    if (error) throw error;
    res.json(data[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.post('/api/campaigns', authMiddleware, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('campaigns').insert([{ name, description, coupon_code, status, gender, min_age, max_age, starts_at, ends_at }]).select();
    if (error) throw error;
    res.status(201).json(data[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.put('/api/campaigns/:id', authMiddleware, async (req, res) => {
  try {
    const { name, description, coupon_code, status, gender, min_age, max_age, starts_at, ends_at } = req.body;
    const { data, error } = await supabase.from('campaigns').update({ name, description, coupon_code, status, gender, min_age, max_age, starts_at, ends_at }).eq('id', req.params.id).select();
    if (error) throw error;
    res.json(data[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.get('/api/audit-logs', authMiddleware, async (req, res) => {
  try {
    ...
    if (search) query = query.or(`username.ilike.%${search}%,type.ilike.%${search}%,object.ilike.%${search}%,action.ilike.%${search}%,ip.ilike.%${search}%,user_agent.ilike.%${search}%,details.ilike.%${search}%`);
    const { data, error } = await query;
    if (error) throw error;
    res.json(data || []);
  } catch (error) {
...
app.get('/api/logs', authMiddleware, async (req, res) => {
  try {
    ...
    if (start_date) query = query.gte('created_at', start_date);
    if (end_date) query = query.lte('created_at', end_date);
    if (search) query = query.or(`message.ilike.%${search}%,source.ilike.%${search}%,level.ilike.%${search}%,ip.ilike.%${search}%,user_agent.ilike.%${search}%,details.ilike.%${search}%`);
    const { data, error } = await query;
    if (error) throw error;
    res.json(data || []);
  } catch (error) {
...
app.get('/api/backup/list', authMiddleware, async (req, res) => {
  try {
    ...
    const files = fs.readdirSync(BACKUP_DIR).filter(f => f.endsWith('.json')).map(f => ({ filename: f, size: fs.statSync(path.join(BACKUP_DIR, f)).size, created_at: fs.statSync(path.join(BACKUP_DIR, f)).birthtime })).sort((a, b) => b.created_at - a.created_at);
    res.json(files);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// ============================================================
// 🔄 ALIAS E ROTAS DE COMPATIBILIDADE PARA O PORTAL
// ============================================================
...
app.post('/api/portal/create-pix', paymentLimiter, async (req, res) => {
  try {
    ...
    if (cleanMac) {
      ...
      if (result?.success) {
        await supabase.from('hotspot_sessions').insert({ mac_address: cleanMac, status: 'active', expires_at: expiresAt, created_at: new Date().toISOString() });
      }
    }
  } catch (_e) {}
...
app.post('/api/portal/login', async (req, res) => {
  try {
    ...
    const { data: user, error } = await supabase.from('users').select('*').or(`username.eq.${identifier},email.eq.${identifier},cpf.eq.${identifier},phone.eq.${identifier}`).single();
...
app.post('/api/portal/register', portalWriteLimiter, async (req, res) => {
  try {
    const { name, cpf, phone, password, mac_address, email, address } = req.body;
    const cleanMac = normalizeMac(mac_address);
    if (cleanMac) {
      const { data: existing } = await supabase.from('users').select('id, username').in('mac_address', getMacVariants(cleanMac)).maybeSingle();
      if (existing) return res.json({ user_id: existing.id, username: existing.username, existing: true });
    }
    const username = cpf || phone || (name ? name.toLowerCase().replace(/\s+/g, '.') : `user_${Date.now()}`);
    const hashedPassword = crypto.createHash('sha256').update(password).digest('hex');
    const { data, error } = await supabase.from('users').insert({ username, name, cpf: cpf || '', phone: phone || '', email, address, password: hashedPassword, mac_address: cleanMac || '', status: 'pending', created_at: new Date().toISOString() }).select().single();
    if (error) throw error;
...
app.get('/api/portal/status', async (req, res) => {
  try {
    ...
    if (session && new Date(session.expires_at) > new Date()) return res.json({ connected: true, expires_at: session.expires_at });
...
app.get('/api/portal/payment-status', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('payments').select('*').in('user_mac', getMacVariants(cleanMac)).order('created_at', { ascending: false }).limit(1).maybeSingle();
    if (error) throw error;
...
// ============================================================
// 🎁 ROTAS DE TESTE GRÁTIS E ACESSO PÚBLICO
// ============================================================
...
app.post('/api/users/test-access', accessLimiter, async (req, res) => {
  try {
    ...
    const out = await handleFreeTrialAccess({ macAddress, ipAddress, popId, popIp });
...
app.post('/api/free-trial', accessLimiter, async (req, res) => {
  try {
    ...
    const out = await handleFreeTrialAccess({ macAddress: mac_address || mac, ipAddress: req.body?.ip_address ?? req.body?.ip ?? null, popId: req.body?.pop_id ?? null, popIp: req.body?.pop_ip ?? null });
...
app.post('/api/access/validate', async (req, res) => {
  try {
    ...
    const now = new Date().toISOString();
    const { data: session, error } = await supabase.from('hotspot_sessions').select('*').in('mac_address', getMacVariants(cleanMac)).eq('status', 'active').gt('expires_at', now).order('created_at', { ascending: false }).limit(1).maybeSingle();
    if (error) throw error;
...
app.get('/api/access/status', async (req, res) => {
  ...
  try {
    ...
    const { data: user } = await supabase.from('users').select('*').in('mac_address', variants).order('updated_at', { ascending: false }).limit(1).maybeSingle();
    if (isActivePaidUser(user)) {
      const planName = user.plan_name || 'Premium';
      const durationForPlan = user.expires_at ? Math.max(10, Math.ceil((new Date(user.expires_at).getTime() - nowMs) / 1000)) : 30 * 24 * 60 * 60;
      const { data: plan } = await supabase.from('plans').select('*').eq('name', planName).maybeSingle();
      await authorizeAccess(cleanMac, popIp || '192.168.32.1', null, null, popId || user.hotspot_id || null, Math.ceil(durationForPlan / 60), plan?.speed_mbps || 10, planName, durationForPlan);
      return res.json({ allowed: true, reason: 'manual_plan_active', show_free_trial: false, expires_at: user.expires_at || null });
    }
    const { data: payment } = await supabase.from('payments').select('*').in('user_mac', variants).in('status', ['approved', 'confirmed', 'pago', 'paid']).order('approved_at', { ascending: false, nullsFirst: false }).order('updated_at', { ascending: false }).limit(1).maybeSingle();
    if (payment) {
      const { data: plan } = await supabase.from('plans').select('*').eq('name', payment.plan_name).maybeSingle();
    ...
    const { data: session } = await supabase.from('hotspot_sessions').select('*').in('mac_address', variants).eq('status', 'active').gt('expires_at', nowIso).order('created_at', { ascending: false }).limit(1).maybeSingle();
    if (session) return res.json({ allowed: true, reason: 'active_session', show_free_trial: false, expires_at: session.expires_at });
    const cfg = await getFreeTrialConfig();
    if (!cfg.enabled) return res.json({ allowed: false, reason: 'no_access', show_free_trial: false });
    const { data: ft } = await supabase.from('free_trials').select('*').in('mac_address', variants).order('updated_at', { ascending: false }).limit(1).maybeSingle();
    if (ft) {
...
app.post('/api/admins', authMiddleware, async (req, res) => {
  try {
    ...
    if (!username || !password) return res.status(400).json({ error: 'Usuário e senha são obrigatórios' });
    const hashedPassword = crypto.createHash('sha256').update(password).digest('hex');
    const { data, error } = await supabase.from('admins').insert([{ username, email, password: hashedPassword, role, created_at: new Date().toISOString() }]).select().single();
    if (error) throw error;
...
app.put('/api/admins/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (password) updateData.password = crypto.createHash('sha256').update(password).digest('hex');
...
// ============================================================
// 🔧 FUNÇÕES FALTANTES (FreeRADIUS sync)
// ============================================================
...
async function syncFreeradiusClientsFromDb() {
  ...
  freeradiusSyncInFlight = (async () => {
    try {
      ...
      await registerSystemLog("error", "FreeRADIUS Sync", "Erro ao sincronizar clientes FreeRADIUS", { error: err.message, stack: err.stack });
      return { ok: false, error: err.message };
    } finally {
