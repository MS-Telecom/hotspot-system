...
try {
  ...
  console.warn('?? node-routeros n?o instalado - recursos de API MikroTik desabilitados');
}
  ...
  freeradiusSyncInFlight = (async () => {
    try {
      ...
      if (!mainClientsConfContent.includes(FREERADIUS_INCLUDE_LINE)) {
        // Se a linha de include n?o existe, adiciona-a
      ...
      if (validateError) throw new Error(`Valida??o FreeRADIUS falhou: ${validateErr || validateOut}`);
...
async function revokeAccess(macAddress, popIp = '192.168.32.1', apiUser = null, apiPass = null, popId = null) {
  if (!RouterOSAPI) {
    console.warn('?? revokeAccess: node-routeros n?o dispon?vel');
    return false;
  }
  ...
  try {
    ...
    if (!username || !password) {
      throw new Error('Credenciais da API MikroTik n?o dispon?veis');
    }
...
async function getFreeTrialConfig() {
  try {
    ...
    await registerSystemLog('error', 'free_trial', 'Erro ao carregar configura??o de teste gr?tis', { error: error.message });
...
async function getPaymentSettings() {
  try {
    ...
    await registerSystemLog('error', 'payment', 'Erro ao carregar configura??o de pagamento', { error: error.message });
...
function getTrialCooldownUntil(record, cfg) {
  ...
  // Reuso deve seguir a configura??o atual do painel. N?o use cooldown_until antigo
  // quando existe expires_at, pois ele pode ter sido gravado com outra configura??o.
...
async function handleFreeTrialAccess({ macAddress, ipAddress = null, popId = null, popIp = null }) {
  ...
  try {
    ...
    await registerSystemLog('error', 'free_trial', 'RADIUS liberado, mas falhou ao gravar sess?o', { mac: cleanMac, error: error.message });
  }
...
// ============================================================
// ?? ROTAS DE AUTENTICA??O (ADMIN)
// ============================================================

// Aliases legados (portugu?s/curtos) -> padr?o novo
...
app.post('/api/users', authMiddleware, async (req, res) => {
  try {
    ...
    if (plan_id) {
      ...
      if (!planData) return res.status(404).json({ error: 'Plano n?o encontrado' });
      plan = planData;
...
app.post('/api/users/:id/renew', authMiddleware, async (req, res) => {
  try {
    ...
    if (planError || !plan) return res.status(400).json({ error: 'Plano n?o encontrado' });
...
app.get('/api/users/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (userError || !user) return res.status(404).json({ error: 'Usu?rio n?o encontrado' });
...
async function ensurePopProvisioningMaterial(pop) {
  ...
  if (nextVpnEnabled && nextVpnType === 'l2tp_ipsec') {
    try {
      ...
      warnings.push(`VPN IP n?o gerada: ${err.message}`);
  ...
  if (credErr) warnings.push(`Credenciais MikroTik n?o lidas: ${credErr.message}`);
...
app.post('/api/pops', authMiddleware, async (req, res) => {
  try {
    ...
    // Guarda configura??o avan?ada (para gerar script completo mesmo se a tabela pops n?o tiver todas as colunas)
    try {
      await supabase.from('settings').upsert({
      ...
      // ignora se settings n?o existir/estruturar diferente
    }
...
function buildPopInstallScript(pop, config = {}) {
  ...
  const vpnBlock = vpnReady && vpnType === 'l2tp_ipsec'
    ...
    : (vpnReady && vpnType === 'sstp'
      ...
      : (vpnEnabled
        ? `# VPN n?o gerada: configura??o incompleta ou modo RADIUS sem VPN\n`
        : ''));
  ...
  const wanBlock = (() => {
    ...
    if (isTrunk) {
      ...
      // Por padr?o, usa DHCP nessa VLAN para obter acesso a Internet/Rotas (via MikroTik principal).
...
app.put('/api/pops/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (existingErr || !existing) return res.status(404).json({ error: 'POP n?o encontrado' });
...
// Gerar script de configura??o para um POP
...
app.get('/api/pops/:id/revert-script', authMiddleware, async (req, res) => {
  try {
    ...
    console.error('? Erro ao gerar script de revers?o:', err.message);
    res.status(500).json({ error: 'Erro ao gerar script de revers?o' });
  }
});
...
app.post('/api/pops/register', async (req, res) => {
  try {
    ...
    if (!name || !ip) return res.status(400).json({ error: 'name e ip s?o obrigat?rios' });

    const popToken = crypto.randomBytes(32).toString('hex');
    // N?o insere `token` na tabela pops porque o schema atual n?o possui essa coluna.
    ...
    // Guarda o token em settings para valida??o futura (se necess?rio)
    try {
      await supabase.from('settings').upsert({
...
app.get('/api/public/free-trial-config', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar configura??o do teste gr?tis' });
  }
});
...
// Rota de edi??o via POST (conforme frontend webhooks.html)
...
app.post('/api/webhooks/:id/test', authMiddleware, async (req, res) => {
  try {
    ...
    if (!webhook) return res.status(404).json({ error: 'Webhook n?o encontrado' });
...
app.post('/api/portal/login', async (req, res) => {
  try {
    ...
    const { data: user, error } = await supabase.from('users').select('*')
      ...
      .single();
    if (error || !user) return res.status(401).json({ error: 'Usu?rio n?o encontrado' });
...
app.post('/api/portal/voucher', async (req, res) => {
  // Encaminha para a rota oficial de valida??o de voucher
...
app.get('/api/payments', async (req, res, next) => {
  ...
  // Se N?O tem token e tem mac_address, redireciona para a rota p?blica do portal
  if (!authHeader && mac_address) {
    req.url = '/api/portal/payment-status';
    return app._router.handle(req, res);
  }

  // Caso contr?rio, segue para o middleware de autentica??o e rota admin
  next();
});
...
// ============================================================
// ?? GEST?O DE ADMINISTRADORES (ADMINS)
// ============================================================
...
app.post('/api/admins', authMiddleware, requireRole('owner'), async (req, res) => {
  try {
    ...
    if (!ADMIN_ROLES.has(role) || role === 'owner') {
      return res.status(403).json({ error: 'Role n?o permitida', reason: 'role_escalation_blocked' });
    }
...
app.put('/api/admins/:id', authMiddleware, requireRole('owner'), async (req, res) => {
  try {
    ...
    if (String(id) === String(req.user.id) && requestedRole) {
      return res.status(403).json({ error: 'N?o ? permitido alterar o pr?prio role', reason: 'self_role_change_blocked' });
    }
...
app.delete('/api/admins/:id', authMiddleware, requireRole('owner'), async (req, res) => {
  try {
    ...
    if (!target) return res.status(404).json({ error: 'Administrador n?o encontrado' });
    if (target.role === 'owner') {
      ...
      if ((count || 0) <= 1) {
        return res.status(409).json({ error: 'N?o ? permitido excluir o ?ltimo owner', reason: 'last_owner_blocked' });
      }
    }
